...
int __system(const char *command)
{
  pid_t pid;
	...
	//prepend command string with "/sbin/busybox"
	char commandstr[255];
	strcpy (commandstr, "/sbin/busybox ");
	strcat (commandstr, command);
	argp[2] = (char *)commandstr;
...
int __systemscript(const char *command)
{
  pid_t pid;
    sig_t intsave, quitsave;
    sigset_t mask, omask;
	...
	//no prepend command string with "/sbin/busybox"
	argp[2] = (char *)command;
...
void show_partition_menu()
{
    ...
    for (;;)
    {
        ...
        /* Midnight: disable nonworking mount options
...
void show_nandroid_menu()
{
    static char* headers[] = {  "Nandroid",
								"Recommended to Backup/Restore DATA only",
                                "",
                                NULL 
    };

    static char* list[] = { "Backup DATA: data + dbdata + cache", 
                            "Restore DATA",
							"Backup All",
        ...
        case 0:
            {
                ...
                nandroid_backup_flags(backup_path,DONT_BACKUP_SYSTEM);
            }
            break;
        case 1:
